...
| --------------- | ---- | -------- | ----------------------------------- |
| `filepath`      | str  | Yes      | Path to the JSON configuration file |
| `sandbox`       | bool | No       | Whether to use sandboxed execution  |
| `sandbox_options` | SandboxOptions | No | Configuration options for sandbox |
...
Both `from_config_file` and `from_dict` methods support sandboxed execution for running MCP servers in isolated environments:
...
| `auto_initialize` | bool  | No       | True    | Whether to automatically initialize the session |

**When to use**:

- Set `auto_initialize=False` if you want to manually initialize the session later
...
agent = MCPAgent(
    ...
    tools_used_names=None,
    use_server_manager=False,
    server_manager=None,
    verbose=False,
    agent_id=None,
    api_key=None,
    base_url="https://cloud.mcp-use.com",
    callbacks=None,
    chat_id=None,
    retry_on_error=True,
...
| Parameter                 | Type                | Required | Default | Description                                                                                                                                     |
| ------------------------- | ------------------- | -------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `llm`                     | BaseLanguageModel   | No       | None    | Any LangChain-compatible language model. Not required if agent_id is provided for remote execution.                                            |
...
| `tools_used_names`        | list[str]           | No       | None    | List of tool names that have been used by the agent                                                                                             |
...
| `server_manager`          | BaseServerManager   | No       | None    | Custom server manager instance                                                                                                                  |
| `verbose`                 | bool                | No       | False   | Enable verbose logging during execution                                                                                                         |
| `agent_id`                | str                 | No       | None    | Remote agent ID for cloud execution                                                                                                             |
| `api_key`                 | str                 | No       | None    | API key for remote execution. If None, checks MCP_USE_API_KEY env var.                                                                          |
| `base_url`                | str                 | No       | "https://cloud.mcp-use.com" | Base URL for remote API calls                                                                                                               |
| `callbacks`               | list                | No       | None    | List of LangChain callback handlers                                                                                                             |
| `chat_id`                 | str                 | No       | None    | Chat ID for conversation continuity across sessions                                                                                             |
| `retry_on_error`          | bool                | No       | True    | Whether to retry tool calls that fail due to validation errors                                                                                  |
| `max_retries_per_step`    | int                 | No       | 2       | Maximum number of retries for validation errors per step                                                                                        |
...
- **Remote execution (agent_id)**:
  - Use when you want to run agents in the cloud without local LLM setup
  - Provide `agent_id` instead of `llm` for cloud execution
  - API key can be provided directly or via `MCP_USE_API_KEY` environment variable

- **Error handling and retries**:
  - `retry_on_error`: Enable automatic retry on validation errors
  - `max_retries_per_step`: Control how many times to retry failed tool calls

- **Verbose logging**:
  - `verbose`: Enable detailed logging during agent execution
  - Helpful for debugging and monitoring agent behavior
  - Shows reasoning, tool calls, and results in detail
...
| `manage_connector`     | bool | No       | True    | Whether to handle connector lifecycle |
| `external_history`     | list[BaseMessage] | No | None | External conversation history |
| `output_schema`        | type[T] | No     | None    | Pydantic model for structured output |

**When to use different parameters**:

- **max_steps**: Override the instance default for specific queries
- **manage_connector**: Set to False when you want to manage connector lifecycle manually
- **external_history**: Use when you want to provide conversation history from another source
- **output_schema**: Use when you need structured output in a specific Pydantic model format
...
Use agents in the cloud without local LLM setup:
...
result = await agent.run("Search for information about Python libraries")

# Streaming works the same way
async for step in agent.stream("What's the weather like?"):
...
Get structured results using Pydantic models:
