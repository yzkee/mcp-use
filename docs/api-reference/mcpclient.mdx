...
Creates a new MCPClient instance from a configuration dictionary or file path.

**Parameters:**
- `config` (str | dict, optional): Either a dict containing configuration or a path to a JSON config file. If None, an empty configuration is used.
- `allowed_servers` (list[str], optional): List of server names that are allowed to be used. If None, all configured servers are allowed.
- `sandbox` (bool, optional): Whether to use sandboxed execution mode for running MCP servers. Defaults to False.
- `sandbox_options` (SandboxOptions, optional): Configuration options for sandbox execution.
- `sampling_callback` (SamplingFnT, optional): Optional sampling callback function.
- `elicitation_callback` (ElicitationFnT, optional): Optional elicitation callback function.
- `message_handler` (MessageHandlerFnT, optional): Optional message handler function.
- `logging_callback` (LoggingFnT, optional): Optional logging callback function.
- `middleware` (list[Middleware], optional): List of middleware to use.
...
client = MCPClient(
    config=config,
    sandbox=True,
    sandbox_options={"api_key": "your_e2b_key"},
...
- `sandbox` (bool, optional): Whether to use sandboxed execution mode. Defaults to False.
- `sandbox_options` (SandboxOptions, optional): Configuration options for sandbox execution.
- `sampling_callback` (SamplingFnT, optional): Optional sampling callback function.
- `elicitation_callback` (ElicitationFnT, optional): Optional elicitation callback function.
- `message_handler` (MessageHandlerFnT, optional): Optional message handler function.
...
```python
def add_server(name, server_config)
```
...
Adds a middleware to the client. Middleware is applied to all sessions created by this client.
...
```python
async def create_session(server_name, auto_initialize=True)
```
...
```python
async def create_all_sessions(auto_initialize=True)
```
